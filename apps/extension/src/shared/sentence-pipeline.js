(function (root, factory) {
  ...
  function privatelyOwnsToken(element, options, expectedRootId) {
    return Boolean(element && typeof options.ownsToken === 'function' &&
      options.ownsToken(element, expectedRootId));
  }

  function isHaloOwned(element, options) {
    if (privatelyOwnsToken(element, options)) return true;
  ...
  function isRemappableHaloToken(element, options) {
    return privatelyOwnsToken(element, options, options.expectedRootId);
  }
  ...
  function unsuitableElement(element, options, sensitiveCache, structurallyHidden) {
    ...
    const privatelyOwned = privatelyOwnsToken(element, options);
    const retainsOwnedOffsets = privatelyOwned && options.includeHaloOwnedTokens === true;
    if (isEditable(element) ||
        (isHaloOwned(element, options) && !isRemappableHaloToken(element, options) && !retainsOwnedOffsets)) return true;
