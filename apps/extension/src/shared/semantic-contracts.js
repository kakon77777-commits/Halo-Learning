(function (root, factory) {
  ...
  root.HaloSemanticContracts = api;
  ...
  function schemaVersionAt(value, path) {
    if (value !== SEMANTIC_SCHEMA_VERSION) fail(path, `must equal ${SEMANTIC_SCHEMA_VERSION}`);
    return value;
  }

  function normalizeIdentity(value, path) {
    const raw = objectAt(value, path);
    return { id: stringAt(raw.id, `${path}.id`), version: stringAt(raw.version, `${path}.version`) };
  }
  ...
  function normalizeSemanticAnnotation(value) {
    ...
    const result = {
      schemaVersion: schemaVersionAt(raw.schemaVersion, 'annotation.schemaVersion'),
    ...
    if (raw.datasetRef !== undefined) result.datasetRef = normalizeDatasetRef(raw.datasetRef, 'annotation.datasetRef');
  ...
  function normalizeSemanticToken(value) {
    ...
    const result = {
      schemaVersion: schemaVersionAt(raw.schemaVersion, 'token.schemaVersion'),
      ...
      annotations: raw.annotations.map(normalizeSemanticAnnotation)
    };
    if (raw.lemma !== undefined) result.lemma = stringAt(raw.lemma, 'token.lemma');
    if (raw.simplifiedPos !== undefined) result.simplifiedPos = enumAt(raw.simplifiedPos, POS_TAGS, 'token.simplifiedPos');
    if (raw.morphology !== undefined) result.morphology = jsonValueAt(objectAt(raw.morphology, 'token.morphology'), 'token.morphology');
  ...
  function normalizeDiagnostics(value) {
    ...
    if (typeof raw.fallbackActivated !== 'boolean') fail('annotationSet.diagnostics.fallbackActivated', 'must be a boolean');
    return {
      fallbackActivated: raw.fallbackActivated,
      unavailableCapabilities: stringsAt(raw.unavailableCapabilities, 'annotationSet.diagnostics.unavailableCapabilities'),
  ...
  function normalizeAnnotationSet(value) {
    ...
    const tokens = raw.tokens.map(normalizeSemanticToken);
    let previousEnd = 0;
    ...
    return deepFreeze({
      schemaVersion: schemaVersionAt(raw.schemaVersion, 'annotationSet.schemaVersion'),
      ...
      providerRefs: raw.providerRefs.map((provider, index) => normalizeProviderRef(provider, `annotationSet.providerRefs[${index}]`)),
      tokens,
      diagnostics: normalizeDiagnostics(raw.diagnostics)
    });
  }

  return Object.freeze({ SEMANTIC_SCHEMA_VERSION, normalizeAnnotationSet });
});
